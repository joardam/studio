...
export default function NovaCandidaturaPage({ searchParams }: { searchParams: { monitoriaAprovada?: string } }) {
  const isMonitoriaBlocked = searchParams?.monitoriaAprovada === 'true';
    ...
    <div className="grid gap-6">
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          {isMonitoriaBlocked ? (
            <div className="flex flex-col items-center justify-center p-6 text-center rounded-lg border bg-destructive/10 text-destructive shadow-sm pointer-events-none">
                <BookOpen className="h-12 w-12 mb-4" />
                <h3 className="text-lg font-semibold">Monitoria</h3>
                <p className="text-xs mt-2 font-medium">Você já possui uma monitoria aprovada.</p>
            </div>
          ) : (
            <Link
