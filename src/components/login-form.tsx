...
export function LoginForm({ profile }: { profile?: string }) {
  const [email, setEmail] = useState("");
  const [view, setView] = useState<'login' | 'register' | 'forgotPassword'>('login');
  const { toast } = useToast();

  const getProfileFromEmail = (currentEmail: string) => {
    if (currentEmail.includes("professor")) return "professor";
    if (currentEmail.includes("coordenador")) return "coordenador";
    if (currentEmail.includes("admin")) return "administrativo";
    return "aluno";
  };

  useEffect(() => {
    if (profile && profileToEmailMap[profile as keyof typeof profileToEmailMap]) {
        setEmail(profileToEmailMap[profile as keyof typeof profileToEmailMap]);
    } else {
        setEmail(""); // Clear email if profile is not valid
    }
  }, [profile]);

  const handleLoginClick = () => {
    sessionStorage.setItem('userProfile', getProfileFromEmail(email));
  };
  ...
  const dashboardUrl = `/dashboard?profile=${getProfileFromEmail(email)}`;
